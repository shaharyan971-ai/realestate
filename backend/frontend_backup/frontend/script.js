...
function showSetupModal() {
  ...
  modal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.85);z-index:10000;display:flex;align-items:center;justify-content:center;padding:20px;font-family:sans-serif;';
  modal.innerHTML = `
    <div style="background:#1c1e1f;border:1px solid #e6b94a;border-radius:18px;max-width:500px;padding:30px;color:#e8e8e8;box-shadow:0 10px 40px rgba(0,0,0,0.8);">
      <h2 style="color:#e6b94a;margin-bottom:15px;font-size:1.4rem;">Database Setup Required</h2>
      <p style="font-size:0.9rem;line-height:1.6;margin-bottom:20px;">
        To enable login, data persistence, and listings, you need to connect your own <strong>Supabase Project</strong>.
      </p>
      <div style="background:rgba(0,0,0,0.2);padding:15px;border-radius:10px;margin-bottom:20px;font-size:0.82rem;color:#ccc;">
        <strong>1.</strong> Sign up at <a href="https://supabase.com" target="_blank" style="color:#e6b94a;">supabase.com</a><br>
        <strong>2.</strong> Create a new project called "RealEstate"<br>
        <strong>3.</strong> Go to <strong>Project Settings -> API</strong><br>
        <strong>4.</strong> Copy your <strong>Project URL</strong> and <strong>Anon Key</strong><br>
        <strong>5.</strong> Paste them into <code>config.js</code> in this folder.
      </div>
      <button onclick="this.parentElement.parentElement.remove()" style="width:100%;padding:12px;background:#e6b94a;border:none;border-radius:10px;font-weight:800;color:#000;cursor:pointer;"> Got it, let's go!</button>
    </div>
  `;
    ...
    <div class="prop-card-body">
      ...
      <div class="prop-actions">
        ${showEdit
          ...
          : `<a href="property-detail?id=${prop.id}" class="btn btn-gold btn-sm">View Details</a>`}
...
async function showSecurityDashboard() {
  ...
  const logRows = log.length ? log.slice(0, 10).map(entry => `
    ...
    <div style="background:#131516;border:1px solid rgba(255,215,0,0.15);border-radius:18px;width:100%;max-width:480px;max-height:85vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,0.7);">
      <div style="padding:24px 24px 0;position:sticky;top:0;background:#131516;z-index:1;border-bottom:1px solid rgba(255,255,255,0.05);padding-bottom:16px;">
        <div style="display:flex;align-items:center;justify-content:space-between;">
          <div>
            <div style="font-family:'Syne',sans-serif;font-size:1.1rem;font-weight:800;color:#e8e8e8;"> Security Center</div>
      ...
      <div style="padding:20px 24px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:20px;">
          <div style="background:#0d1200;border:1px solid rgba(34,197,94,0.2);border-radius:10px;padding:12px;">
            ...
            <div style="font-size:0.7rem;color:#666">Email OTP on every login</div>
          </div>
          ...
          <div style="background:#0d1200;border:1px solid rgba(34,197,94,0.2);border-radius:10px;padding:12px;">
            <div style="font-size:20px;margin-bottom:4px;"></div>
            <div style="font-size:0.78rem;font-weight:700;color:#22c55e">Auto-Logout</div>
            <div style="font-size:0.7rem;color:#666">30 min idle timeout</div>
          </div>
          <div style="background:#0d1200;border:1px solid rgba(34,197,94,0.2);border-radius:10px;padding:12px;">
            <div style="font-size:20px;margin-bottom:4px;"></div>
            <div style="font-size:0.78rem;font-weight:700;color:#22c55e">Account Lock</div>
            <div style="font-size:0.7rem;color:#666">5 attempts / 15 min</div>
          </div>
        </div>
        ...
        <div style="background:#0d0f10;border:1px solid rgba(255,255,255,0.06);border-radius:10px;padding:14px;margin-bottom:20px;">
          ...
          <div style="font-size:0.8rem;color:#bbb;line-height:1.8;">
            ...
            <div> Role: <span style="color:#e6b94a">${(user.role||'buyer').charAt(0).toUpperCase()+(user.role||'buyer').slice(1)}</span></div>
            <div> Timeout: <span style="color:#e8e8e8">30 min inactivity</span></div>
            <div> Browser: <span style="color:#e8e8e8">${navigator.userAgent.includes('Chrome') ? 'Chrome' : navigator.userAgent.includes('Firefox') ? 'Firefox' : navigator.userAgent.includes('Safari') ? 'Safari' : 'Browser'}</span></div>
          </div>
        </div>
...
//  AUTO-START SessionGuard on every page 
